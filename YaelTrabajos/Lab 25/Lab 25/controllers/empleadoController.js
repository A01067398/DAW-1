...
exports.addAjax = (request, response, next) => {
    response.render("empleado/add-ajax");
};
...
exports.listAjax = (request, response, next) => {
    response.render("empleado/list-ajax");
};

exports.getAllAjax = (request, response, next) => {
    response.setHeader('Content-type', 'text/json');
    Empleado.fetchAll()
    .then(([empleados, fieldData]) => {
        response.send({"response": "success", "data": {"empleados": empleados}});
    })
    .catch((err) => {
        console.log(err)
        response.send({"response": "error", "error": err});
    });
};
